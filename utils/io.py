...
def get_gwtc_skymap_path(
    ...
    ## if in GWTC2.1 (those in GWTC2 from Graham+23 updated to this release)
    if gweventname in [
        "GW190408_181802*",
        "GW190412*",
        "GW190413_052954",
    ...
    if gweventname == "GW190521":
        globstr = f"{mapdir}/{catdir}/*{gweventname}_030229*{waveform}*.fits"
    ...
    ## Special case: GW191219_163120, GW200115_042309 (NSBH --> high/low spin waveforms)
        ...
        globstr = f"{mapdir}/{catdir}/*{gweventname.strip('*')}*{waveform}*.fits"
    mappaths = glob.glob(globstr)

    # If multiple skymaps are found
    if len(mappaths) > 1:
        raise ValueError(
            f"Multiple skymaps found for {gweventname} [globstr: {globstr}]"
        )
    elif len(mappaths) == 0:
...
def get_gwtc_skymap_path_graham23(
    mapdir,
    gweventname,
    catdirs={
        "GWTC2": "all_skymaps",
